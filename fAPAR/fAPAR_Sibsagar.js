...
var min = NDVI.reduceRegion(ee.Reducer.min(), table, 20).values();
print(min, 'NDVI_Min');
var max = NDVI.reduceRegion(ee.Reducer.max(), table, 20).values();
print(max, 'NDVI_max');
...
var FPAR = NDVI.expression (
  '((NDVI-(-0.20))*(0.95-0.05)/((0.72-(-0.20))))+0.05',{
...
var min = FPAR.reduceRegion(ee.Reducer.min(), table, 20).values();
print(min, 'FPAR_Min');
var max = FPAR.reduceRegion(ee.Reducer.max(), table, 20).values();
print(max, 'FPAR_max');
