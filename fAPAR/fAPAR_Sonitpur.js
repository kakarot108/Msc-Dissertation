...
var min = NDVI.reduceRegion(ee.Reducer.min(), table, 30).values();
print(min, 'NDVI_Min');
var max = NDVI.reduceRegion(ee.Reducer.max(), table, 30).values();
print(max, 'NDVI_max');

// FPAR CALCULATION
var FPAR = NDVI.expression (
  '((NDVI-(-0.32))*(0.95-0.05)/((0.72-(-0.32))))+0.05',{
...
//Calculating min and max values:
var min = FPAR.reduceRegion(ee.Reducer.min(), table, 30).values();
print(min, 'FPAR_Min');
var max = FPAR.reduceRegion(ee.Reducer.max(), table, 30).values();
print(max, 'FPAR_max');
